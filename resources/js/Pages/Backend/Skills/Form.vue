...
const props = defineProps(["skills", "id"]);

const form = useForm({
    title: props.skills?.title ?? "",
    percent: props.skills?.percent ?? "",
    image: props.skills?.image ?? "",
    file: props.skills?.file ?? "",


    imagePreview: props.skills?.image ?? "",
    filePreview: props.skills?.file ?? "",
    _method: props.skills?.id ? "put" : "post",
});
...
const submit = () => {
    const routeName = props.id
        ? route("backend.skills.update", props.id)
        : route("backend.skills.store");
    form.transform((data) => ({
        ...data,
        remember: "",
        isDirty: false,
